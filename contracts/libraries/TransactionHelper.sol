...
import {ETH_TOKEN_SYSTEM_CONTRACT, BOOTLOADER_FORMAL_ADDRESS} from "@matterlabs/zksync-contracts/l2/system-contracts/Constants.sol";
...
/// @dev This file is forked from https://github.com/matter-labs/era-contracts/blob/main/system-contracts/contracts/libraries/TransactionHelper.sol,
/// @dev changing the domain name from 'zkSync' to 'Abstract'
...
library TransactionHelper {
    ...
    function _encodeHashEIP712Transaction(
        ...
        bytes32 domainSeparator = keccak256(
            abi.encode(
                ...
                keccak256("Abstract"),  // This is the primary change in the file
                keccak256("2"),
                block.chainid
            )
        );
    ...
    function _encodeHashLegacyTransaction(
        ...
        bytes memory encodedNonce = RLPEncoder.encodeUint256(
            _transaction.nonce
        );
        ...
        bytes memory encodedTo = RLPEncoder.encodeAddress(
            address(uint160(_transaction.to))
        );
        bytes memory encodedValue = RLPEncoder.encodeUint256(
            _transaction.value
        );
        ...
        if (_transaction.reserved[0] != 0) {
            encodedChainId = bytes.concat(
                RLPEncoder.encodeUint256(block.chainid),
                hex"80_80"
            );
        }
    ...
    function _encodeHashEIP2930Transaction(
        ...
        {
            bytes memory encodedChainId = RLPEncoder.encodeUint256(
                block.chainid
            );
            bytes memory encodedNonce = RLPEncoder.encodeUint256(
                _transaction.nonce
    ...
    function _encodeHashEIP1559Transaction(
        ...
        bytes memory encodedFixedLengthParams;
        {
            ...
            bytes memory encodedMaxFeePerGas = RLPEncoder.encodeUint256(
                _transaction.maxFeePerGas
            );
            bytes memory encodedGasLimit = RLPEncoder.encodeUint256(
                _transaction.gasLimit
